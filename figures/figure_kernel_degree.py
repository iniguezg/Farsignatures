...
if __name__ == "__main__":
	...
	#filter variables
	filt_rule = 'degree' #chosen filter parameter ('degree', 'dispersion')
	min_negos = 10 #minimum number of egos in filtered activity group
	...
	dset_params = { 'call': {'min_degree':10, 'num_quants':5},
					...
					'CNS_calls': {'min_degree':2, 'num_quants':3},
					'CNS_sms': {'min_degree':2, 'num_quants':3},}
	...
	# for grid_pos, (eventname, textname) in enumerate([ ( 'fb', 'Facebook') ]):
		...
		for posval, (min_val, max_val) in enumerate( zip(quantile_vals[:-1], quantile_vals[1:]) ):
			print('\tmin_val = {}, max_val = {}'.format(min_val, max_val), flush=True) #filter range
			...
			label = '{:.0f} '.format(min_val)+'$\leq k <$'+' {:.0f}'.format(max_val)
			plt.plot( data_avg - bline_avg, '-', c=colors[posval], label=label, lw=plot_props['linewidth'], zorder=1 )
