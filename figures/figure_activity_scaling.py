...
if __name__ == "__main__":
	...
	'fig_size' : (12, 8),
	'aspect_ratio' : (2, 3),
	'grid_params' : dict( left=0.065, bottom=0.08, right=0.985, top=0.95, wspace=0.1, hspace=0.5 ),
	'dpi' : 300,
...
#THEO
		...
		plt.text( 0.5, 1.1, reg_str+r'($\alpha_r=$ {:.1f}'.format(gamma)+')', va='top', ha='center', transform=ax.transAxes, fontsize=plot_props['ticklabel'] )
		...
		#legends
		if alphapos == 1:
			leg = plt.legend( loc='upper center', bbox_to_anchor=(0.5, -0.22), prop=plot_props['legend_prop'], handlelength=plot_props['legend_hlen'], numpoints=plot_props['legend_np'], columnspacing=plot_props[ 'legend_colsp' ], ncol=len(t_vals) )
		if alphapos == 0:
			leg = plt.legend( (line_sims, line_theo, line_gamma), ('num', 'Eq. (S9)', 'Eq. ()'), loc='upper right', bbox_to_anchor=(1, 0.9), prop=plot_props['legend_prop'], handlelength=2.2, numpoints=plot_props['legend_np'], columnspacing=plot_props[ 'legend_colsp' ] )
